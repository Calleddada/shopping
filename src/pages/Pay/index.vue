...
import QRCode from 'qrcode'
  export default {
    ...
    async open() {
      try {
        // 异步生成 QR 码
        const url = await this.generateQRCode('https://www.baidu.com');
        // 安全地展示 QR 码图片
        this.showPaymentQRCode(url);
        
      } catch (err) {
        console.error('生成或显示 QR 码时出错:', err);
        // 可以考虑进一步的错误处理，例如向用户显示错误消息
      }
    },
    ...
    async generateQRCode(url) {
      return new Promise((resolve, reject) => {
        ...
        throw new Error('生成 QR 码失败:' + err.message);
      });
    },

    // 安全地显示 QR 码图片
    showPaymentQRCode(url) {
      // 使用 Vue 的插槽或其它安全方法插入动态内容，这里简单使用了模板字符串
      const safeHTML = `<img src="${url}">`;
      
      this.$alert(safeHTML, '请你微信支付', {
